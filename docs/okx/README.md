...
| [`pricing.md`](pricing.md) | Real measured costs: 0G Compute (personality generation) and 0G Chain gas (INFT mint, via a read-only `estimateGas` simulation). What's still unknown: 0G Storage upload cost and the 0G→USD rate. |
...
| [`../../services/okx-payment-proxy/`](../../services/okx-payment-proxy/README.md) | Real, typechecked reverse-proxy scaffold (verified against the actual `mppx`/`@okxweb3/mpp` types) that will pay-wall the endpoint. Not deployed — refuses to start until real pricing + OKX credentials exist. |
...
- Two of three pricing components are real measurements (personality generation, INFT mint gas).
- Payment proxy (`services/okx-payment-proxy`) is built, typechecks, and is installed in the
  workspace — but not deployed anywhere, and won't start without real credentials/pricing.
- **Not yet done**: applying the migration to the live Render database, setting `OKX_SERVICE_KEY`
  in Render's dashboard, measuring the last pricing unknown (0G Storage cost) or sourcing it
  externally, registering as an OKX ASP to get real payment credentials, wiring
