...
| 0 | **Found + fixed three stacked League bugs while wiring this**, each independently causing the same symptom (silent fallback to `"Agent is thinking..."`): (1) `/league-prediction` route was never registered in `inference-service` (fixed, `8469d3b`); (2) once reachable, `decideLeaguePrediction` crashed with `PrismaClientInitializationError` because `aiarena-inference` never had `DATABASE_URL` configured — user added it manually via Render's Environment tab, confirmed fixed by logs; (3) with that fixed, the real 0G Compute call itself then timed out at the original 12s budget (`zai-org/GLM-5.1-FP8` is reasoning-capable and needs longer for a structured tool-call response) — raised to 25s (`e6faf98`). Final live verification (a fresh pick showing `source: 'AI'`) still pending the redeploy of this last fix. | 🟡 All three fixes pushed, final live verification pending |
...
**Pending step (blocks Phase 1 completion):** `PolymarketSignal` migration is applied and `DATABASE_URL` is now set on `aiarena-inference` — both done. Waiting on `aiarena-inference` to redeploy with the 25s timeout fix (`e6faf98`), then one fresh (never-predicted) League pick to confirm the response shows `"source": "AI"` instead of `"source": "FALLBACK"` / `"reasoning": "Agent is thinking..."`.
