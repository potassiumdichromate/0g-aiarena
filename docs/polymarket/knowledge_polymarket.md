...
| 0 | **Found + fixed four stacked League bugs while wiring this**, each independently causing the same symptom (silent fallback to `"Agent is thinking..."`): (1) `/league-prediction` route never registered in `inference-service` (`8469d3b`); (2) `aiarena-inference` missing `DATABASE_URL`, crashing `decideLeaguePrediction`'s own DB lookup before its 0G Compute try/catch was even reached — user fixed via Render's Environment tab; (3) the real 0G Compute call then timed out at the original 12s budget — raised to 25s (`e6faf98`); (4) with timeout fixed, logs showed `"No parseable league prediction in 0G Compute response"` — the configured model (`zai-org/GLM-5.1-FP8`) is a reasoning/"thinking" model (already documented in `compute.client.ts`'s own comments as prepending chain-of-thought), and the 300-token budget was almost certainly too tight to fit both the reasoning and the final structured answer, truncating before any parseable JSON. Raised to 1200 (`17c7ce6`). Final live verification (a fresh pick showing `source: 'AI'`) still pending this redeploy. | 🟡 All four fixes pushed, final live verification pending |
...
**Pending step (blocks Phase 1 completion):** `PolymarketSignal` migration applied, `DATABASE_URL` set on `aiarena-inference`, timeout raised to 25s, max_tokens raised to 1200 — all done/pushed. Waiting on `aiarena-inference`'s redeploy of `17c7ce6`, then one fresh (never-predicted) League pick to confirm the response shows `"source": "AI"` instead of `"source": "FALLBACK"` / `"reasoning": "Agent is thinking..."`.
