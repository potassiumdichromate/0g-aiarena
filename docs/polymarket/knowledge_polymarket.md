...
- **Polymarket's Gamma API is currently unreachable from real production browser sessions**, not just this sandbox — confirmed by the user hitting a fallback market (exact hardcoded question + volume figure) live on app.kult.games. Root cause not yet diagnosed (could be CORS, rate-limiting, or a Polymarket-side issue) — needs checking from an actual browser's Network tab, which this environment can't do. Until that's root-caused, the whole Market/Analysis tab experience may be running on fallback data for real users far more often than intended. Mitigated for now (see below) but not fixed at the source.
